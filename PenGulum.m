...
hGui.controlText(11) = uicontrol(hGui.Fig,'Style','text',...
    'String','Fs','Position',[765 620-(140) 130 20]);
...
hGui.controlText(12) = uicontrol(hGui.Fig,'Style','text',...
    'String','k','Position',[765 620-(210) 130 20]);

hGui.controlEdit(12) = uicontrol(hGui.Fig,'Style','edit',...
    'Position',[765 600-(210) 130 20],'Tag','12',...
    'Callback',@changeValue2);

hGui.controlSlide(12) = uicontrol(hGui.Fig,'Style','slider',...
    'Position',[765 580-(210) 130 20],...
    'value',0, 'min',0, 'max',10,'SliderStep',[.01 .1],...
    'Tag','12','Callback',@changeValue);

hGui.controlText(13) = uicontrol(hGui.Fig,'Style','text',...
    'String','kSRS','Position',[765 620-(280) 130 20]);

hGui.controlEdit(13) = uicontrol(hGui.Fig,'Style','edit',...
    'Position',[765 600-(280) 130 20],'Tag','13',...
    'Callback',@changeValue2);

hGui.controlSlide(13) = uicontrol(hGui.Fig,'Style','slider',...
    'Position',[765 580-(280) 130 20],...
    'value',0, 'min',0, 'max',100,'SliderStep',[.01 .1],...
    'Tag','13','Callback',@changeValue);
...
hGui.Save = uicontrol('style','pushbutton','string','Save figure',...
    'Position',[485 20 130 40],'Callback',@savefigure);
...
data.model.Fs = 120;
data.model.k = 0;
data.model.kSRS = 38.1972;

data.model.vectorID = {'m','lc','I','Tb','d','dur','kF','kdF','q0','scale','Fs','k','kSRS'};
...
%% Callbacks
    ...
    function simulation(hObject,~)        
        ...
        tspan = [1/Fs dur]';
        inputdata.k = handle.model.vector(strcmp(cellstr(handle.model.vectorID),'k'));
        inputdata.kSRS = handle.model.vector(strcmp(cellstr(handle.model.vectorID),'kSRS'));
        ...
        switch handle.model.flag
            case {1,4}
                hist = 1;
                [tM,sim] = ode15s(@pendulumStateDerivative_SRS, tspan(1):1/Fs:tspan(2), x0, options, inputdata);
        ...
        handle.simulation.time = tspan(1):1/Fs:tspan(2);
        set(gcf, 'pointer', 'arrow')
        guidata(hObject, handle)
    end
    ...
    function savefigure(hObject,~)
         handle = guidata(hObject);
         temp.model.vector = handle.model.vector;
         temp.model.vectorID = handle.model.vectorID;
         temp.simulation = handle.simulation;
         temp.recorded = handle.recorded;
         assignin('base','subj',temp)
    end
end